...
def pv_peak_dec_array_TCH(x, x0, Tc, h, x1):
    ...
    bg = (4*np.log(2))/(H**2)
    x0=x0+((h/R)*np.sin((x0)*(np.pi/180)))*(180/np.pi)
    x= x.reshape((len(x),1))
...
def MP_intermediate(list_tth, list_db, list_icor, sizeTest, l_comb):
    ...
    y_Tc_constant = np.array([[421.74, 87.32, 99999.00, 99999.00, 396.64, 365.49, 24177.92, 99999.00, 711.51, 99999.00, 314.71]])
    # =============================================================================
    #     # First all is free
    # =============================================================================
...
def MP_intermediate_1(list_tth, list_db, list_icor, sizeTest, l_combo):
    ...
    y_Tc = np.array([[421.74, 87.32, 99999.00, 99999.00, 396.64, 365.49, 24177.92, 99999.00, 711.51, 99999.00, 314.71]])
    ...
    for _ in range(sizeTest):
        ...
        y_pp[0, lc0] = np.random.randint(900, 1000) / 1000
        y_pp = np.round(y_pp/y_pp.sum(axis=1), 3) # Sum of list = 1
